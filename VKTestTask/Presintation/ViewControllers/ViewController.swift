...
//                    description: repoObject.descriptionText,
//                    full_name: repoObject.fullName,
//                    created_at: repoObject.createdAt,
//                    owner: Repository.Owner(avatar_url: repoObject.avatarURL)
//                )
//            }
            tableView.reloadData()
    }
    
    /// Обновление кнопок в NavigationBar
        ...
        func searchBarTextDidBeginEditing(_ searchBar: UISearchBar) {
            // Убираем кнопки, когда начинается поиск
            updateNavigationButtons(visible: false)
        }
        
        func searchBarCancelButtonClicked(_ searchBar: UISearchBar) {
            // Восстанавливаем кнопки при нажатии Cancel
            updateNavigationButtons(visible: true)
        }
        
        func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
            // Действия при нажатии Search (можно оставить пустым или восстановить кнопки)
            updateNavigationButtons(visible: true)
        }
...
//            let repoToDelete = repositories[indexPath.row]
//            repositories.remove(at: indexPath.row)
//            storage.deleteRepository(repoToDelete)
...
//    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        //let selectedRepository = repositories[indexPath.row] // RepositoryObject
        editVC.repository = repositories[indexPath.row]
        editVC.editRepositoryUseCase = self.editRepositroiesUseCase
//        editVC.repository = selectedRepository
//
//        if let repositoryObject = storage.getRepoFromRealm(selectedRepository) {
//            editVC.repositoryObject = repositoryObject
//        } else {
//            print("RepositoryObject not found in Realm for selected repository")
//            return
//        }
